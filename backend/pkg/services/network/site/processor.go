...
func (p *ImportProcessor) Execute(ctx *actions.TaskContext, inputs map[string]string) (map[string]string, error) {
	...
	var newEntries []sitemodel.SitePoolEntry
	if format == "text" {
		...
		for _, e := range parsedEntries {
			...
			newEntries = append(newEntries, sitemodel.SitePoolEntry{
				Type:  e.Type,
				Value: val,
				Tags:  tags,
			})
...
func parseTextRule(line string) sitemodel.SitePoolEntry {
	if strings.HasPrefix(line, "full:") {
		return sitemodel.SitePoolEntry{Type: 3, Value: strings.TrimPrefix(line, "full:")}
	}
	if strings.HasPrefix(line, "domain:") {
		return sitemodel.SitePoolEntry{Type: 2, Value: strings.TrimPrefix(line, "domain:")}
	}
	if strings.HasPrefix(line, "keyword:") {
		return sitemodel.SitePoolEntry{Type: 0, Value: strings.TrimPrefix(line, "keyword:")}
	}
	if strings.HasPrefix(line, "regexp:") {
		return sitemodel.SitePoolEntry{Type: 1, Value: strings.TrimPrefix(line, "regexp:")}
	}
	// Default to domain
	return sitemodel.SitePoolEntry{Type: 2, Value: line}
}
