...
func (r *Reader) Next() (sitemodel.SitePoolEntry, error) {
	var ruleType uint8
	if err := binary.Read(r.r, binary.LittleEndian, &ruleType); err != nil {
		return sitemodel.SitePoolEntry{}, err
	}
	...
	for i := uint8(0); i < tagCount; i++ {
		...
		if idx >= uint32(len(r.dictionary)) {
			return sitemodel.SitePoolEntry{}, ErrCorruptedData
		}
		tags[i] = r.dictionary[idx]
	}

	return sitemodel.SitePoolEntry{
		Type:  ruleType,
		Value: string(valBuf),
		Tags:  tags,
	}, nil
