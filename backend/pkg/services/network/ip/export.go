...
func (m *ExportManager) TriggerExport(ctx context.Context, exportID string, format string) (string, error) {
	...
	for _, t := range tasks {
		t.mu.Lock()
		match := t.Checksum == currentChecksum && t.Status == shared.TaskStatusSuccess
		tID := t.ID
		tFormat := t.Format
		t.mu.Unlock()
		if match {
...
func (m *ExportManager) runExport(bgCtx context.Context, taskID string, e *ipmodel.IPExport) {
	defer m.wg.Done()
