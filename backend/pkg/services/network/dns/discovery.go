...
func RegisterDiscovery() {
	registerDiscoveryOnce.Do(func() {
		...
		_ = registryruntime.Default().RegisterLookup("network/dns/domains", func(ctx context.Context, search string, cursor string, limit int) (*shared.PaginationResponse[discoverymodel.LookupItem], error) {
			resp, err := dnsrepo.DomainRepo.List(ctx, "", 10000, nil)
			if err != nil {
				return nil, err
			}
		...
		_ = registryruntime.Default().RegisterLookup("network/dns/records", func(ctx context.Context, search string, cursor string, limit int) (*shared.PaginationResponse[discoverymodel.LookupItem], error) {
