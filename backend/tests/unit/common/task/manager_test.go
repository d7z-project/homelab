...
func (m *MockTask) SetStatus(s string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	m.Status = s
}
...
func TestTaskManager_Lifecycle_And_Run(t *testing.T) {
	...
	taskID := "task-1"
	task := &MockTask{ID: taskID, Status: "Pending", CreatedAt: time.Now()}
	...
	manager.RunTask(context.Background(), taskID, func(ctx context.Context, task *MockTask) error {
		...
		assert.Equal(t, "Running", task.GetStatus())
		return nil
	})

	retrieved, _ = manager.GetTask(taskID)
	assert.Equal(t, "Success", retrieved.GetStatus())
}
...
func TestTaskManager_Run_FailureAndPanic(t *testing.T) {
	...
	taskID := "task-panic"
	task := &MockTask{ID: taskID, Status: "Pending"}
	manager.AddTask(task)
...
func TestTaskManager_Cancellation(t *testing.T) {
	...
	retrieved, _ := manager.GetTask(taskID)
	assert.Equal(t, "Cancelled", retrieved.GetStatus())
}
...
func TestTaskManager_ReconcileZombieTasks(t *testing.T) {
	...
	taskID := "task-zombie"
	task := &MockTask{ID: taskID, Status: "Running"}
	manager.AddTask(task)
...
func TestTaskManager_CleanupHooks(t *testing.T) {
	...
	// 构造一个很久以前过期的任务
	taskID := "old-task"
	task := &MockTask{ID: taskID, Status: "Success", CreatedAt: time.Now().Add(-48 * time.Hour)}
	manager.AddTask(task)
