...
@Component({
  ...
  template: `
    ...
    <mat-dialog-content class="flex flex-col !pb-4" style="min-width: 350px; max-width: 600px; height: 500px;">
      <!-- Search Field -->
      <div class="pt-2 pb-2">
        <mat-form-field appearance="outline" class="w-full" subscriptSizing="dynamic">
          <mat-label>搜索名称或 ID</mat-label>
          <mat-icon matPrefix class="opacity-50 ml-2 mr-1">search</mat-icon>
          <input matInput [(ngModel)]="search" (ngModelChange)="onSearchChange($event)" placeholder="输入关键字..." autocomplete="off" />
        </mat-form-field>
      </div>
        ...
        <mat-selection-list [multiple]="false" class="animate-in fade-in duration-500">
            ...
            <mat-list-option 
              ...
              <div class="flex items-center gap-3 py-1.5">
                ...
                <div class="flex flex-col min-w-0 leading-tight gap-0.5">
                  <span class="text-sm font-bold text-on-surface truncate">{{ data.allOptionLabel || '全部' }}</span>
                  <span class="text-[10px] font-mono text-outline opacity-40 truncate">ALL_RESOURCES</span>
                  <span class="text-[10px] text-outline opacity-30 truncate">显示所有项，不进行过滤</span>
                </div>
              </div>
            </mat-list-option>
          }
          ...
          @for (item of items(); track item.id) {
            <mat-list-option [value]="item" 
              ...
              class="mb-0.5 !h-auto">
              <div class="flex items-center gap-3 py-1.5">
                <mat-icon class="text-outline opacity-40 shrink-0">{{ item.icon || 'category' }}</mat-icon>
                <div class="flex flex-col min-w-0 leading-tight gap-0.5">
                  <span class="text-sm font-bold text-on-surface truncate">{{ item.name }}</span>
                  <span class="text-[10px] font-mono text-outline opacity-50 truncate">{{ item.id }}</span>
                  @if (item.description) {
                    <span class="text-[10px] text-outline opacity-30 truncate">{{ item.description }}</span>
                  }
                </div>
              </div>
            </mat-list-option>
