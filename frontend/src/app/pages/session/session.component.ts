          ...
          <div class="bg-surface border border-outline-variant rounded-[32px] overflow-hidden shadow-sm transition-shadow hover:shadow-md">
            <table mat-table [dataSource]="sessions()" class="w-full bg-transparent">
              ...
              <ng-container matColumnDef="ip">
                <th mat-header-cell *matHeaderCellDef class="!pl-8 bg-surface-container-low font-bold text-on-surface-variant uppercase tracking-wider text-[11px]"> IP 地址 </th>
                <td mat-cell *matCellDef="let element" class="!pl-8 py-4 font-mono text-sm text-primary">
                  {{ element.ip || 'Unknown' }}
                  @if (element.id === uiService.sessionId()) {
                    <span class="ml-2 px-1.5 py-0.5 rounded bg-primary/10 text-primary text-[9px] font-bold uppercase">Current</span>
                  }
                </td>
              </ng-container>
              ...
              <ng-container matColumnDef="userAgent">
                <th mat-header-cell *matHeaderCellDef class="bg-surface-container-low font-bold text-on-surface-variant uppercase tracking-wider text-[11px]"> 浏览器 / 设备 </th>
                <td mat-cell *matCellDef="let element" class="py-4">
                  <span class="px-2.5 py-1 rounded-lg bg-surface-container-high text-on-surface text-xs font-medium border border-outline-variant/50" [matTooltip]="element.userAgent">
              ...
              <ng-container matColumnDef="createdAt">
                <th mat-header-cell *matHeaderCellDef class="bg-surface-container-low font-bold text-on-surface-variant uppercase tracking-wider text-[11px]"> 创建时间 </th>
              ...
              <ng-container matColumnDef="actions">
                <th mat-header-cell *matHeaderCellDef class="!pr-8 bg-surface-container-low text-right"></th>
                <td mat-cell *matCellDef="let element" class="!pr-8 text-right">
                  <button mat-icon-button color="warn" (click)="revokeSession(element)" 
                    ...
                    [matTooltip]="element.id === uiService.sessionId() ? '当前会话不可吊销' : '吊销此会话'"
              ...
              <tr class="mat-mdc-row" *matNoDataRow>
                <td class="mat-mdc-cell py-24 text-center" [attr.colspan]="displayedColumns().length">
...
export class SessionComponent implements OnInit, OnDestroy {
  ...
  async loadSessions() {
    ...
    try {
      ...
      this.snackBar.open('加载会话失败', '重试', { duration: 3000 })
        .onAction().subscribe(() => this.loadSessions());
    } finally {
