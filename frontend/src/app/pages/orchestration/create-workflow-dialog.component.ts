...
import { OrchestrationService, RbacService, ModelsWorkflow, ModelsStep, ModelsStepManifest, ModelsParamDefinition, ModelsServiceAccount, ModelsVarDefinition } from '../../generated';
          ...
          <mat-stepper orientation="vertical" #stepper class="!bg-transparent">
            ...
            <mat-step [stepControl]="infoForm">
              ...
              <form [formGroup]="infoForm" class="flex flex-col gap-4 mt-6 max-w-2xl">
                ...
                <mat-form-field appearance="outline">
                  ...
                  <textarea matInput formControlName="description" placeholder="对该工作流的简要说明" rows="2"></textarea>
                </mat-form-field>
            ...
            <mat-step>
              ...
              <div class="flex flex-col gap-4 mt-6 max-w-3xl pb-8">
                <p class="text-sm text-outline mb-2">声明工作流执行时需要接收的变量（如 Webhook 传参）。可以在后续步骤中引用它们。</p>
                
                @for (v of vars.controls; track $index) {
                  @let vIndex = $index;
                  <div [formGroup]="getVarGroup(vIndex)" class="flex gap-4 items-start bg-surface-container-low p-4 rounded-xl border border-outline-variant/30">
                    ...
                    <button mat-icon-button color="warn" (click)="removeVar(vIndex)" matTooltip="删除变量" type="button" class="mt-1">
                  ...
                  <div class="bg-surface border border-outline-variant rounded-2xl overflow-hidden shadow-sm transition-shadow hover:shadow-md relative">
                    ...
                    <div class="bg-surface-container-low px-6 py-3 flex justify-between items-center border-b border-outline-variant/30">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm font-bold opacity-80">
                                {{ getStepGroup(stepIndex)?.get('name')?.value || getStepGroup(stepIndex)?.get('id')?.value || '未命名步骤' }}
                            </span>
                        </div>
                        <button mat-icon-button color="warn" (click)="removeStep(stepIndex)" matTooltip="删除此步骤" class="!w-8 !h-8">
                    ...
                    <div [formGroup]="getStepGroup(stepIndex)!" class="p-6 flex flex-col gap-4">
                      ...
                      <mat-form-field appearance="outline" class="w-full">
                        ...
                        <input matInput formControlName="if" placeholder="例如：steps.step1.outputs.code == '200'">
                      ...
                      <mat-form-field appearance="outline">
                        ...
                        <mat-select formControlName="type" (selectionChange)="onProcessorChange(stepIndex)">
                      ...
                      <!-- Dynamic Parameters -->
                      @if (getProcessorParams(getStepGroup(stepIndex)?.get('type')?.value).length > 0) {
                        <div class="mt-2 space-y-4">
                            ...
                            <div formGroupName="params" class="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-4">
                              @for (param of getProcessorParams(getStepGroup(stepIndex)?.get('type')?.value); track param.name) {
                                <mat-form-field appearance="outline">
                                  <mat-label>{{ param.name }}{{ param.optional ? ' (可选)' : '' }}</mat-label>
                                  <input matInput [formControlName]="param.name!" [matAutocomplete]="auto">
