import { Component, Inject, OnInit, OnDestroy, ViewChild, ElementRef, inject, signal, computed } from '@angular/core';
...
import { OrchestrationService, ModelsTaskInstance, ModelsLogEntry, ModelsWorkflow } from '../../generated';
...
@Component({
  ...
  template: `
    <div class="flex flex-col h-full bg-surface overflow-hidden">
      ...
      <header class="flex items-center justify-between px-3 sm:px-6 h-14 sm:h-16 border-b border-outline-variant/20 bg-surface shrink-0">
        <div class="flex items-center gap-2 sm:gap-4 min-w-0 flex-1">
          <div class="flex flex-col min-w-0">
            <div class="flex items-center gap-1.5 sm:gap-2">
              <span class="text-[8px] sm:text-[10px] px-1.5 py-0.5 rounded font-mono bg-primary/10 text-primary border border-primary/20 shrink-0">#{{ displayId() }}</span>
              <h2 class="text-xs sm:text-base font-bold tracking-tight m-0 truncate">{{ instance().workflowId }}</h2>
            </div>
            <div class="flex items-center gap-1.5 sm:gap-2 text-[9px] sm:text-[11px] text-outline mt-0.5 overflow-hidden">
              <mat-icon class="!w-2.5 !h-2.5 sm:!w-3 sm:!h-3 !text-[10px] sm:!text-[12px] shrink-0" [style.color]="getStatusColor(instance().status)">{{ getStatusIcon(instance().status) }}</mat-icon>
              <span class="font-medium uppercase tracking-tighter shrink-0">{{ instance().status }}</span>
        ...
        <div class="flex items-center gap-1 sm:gap-2 shrink-0 ml-2">
          @if (instance().status === 'Running') {
            <button mat-icon-button color="warn" (click)="cancel()" class="sm:hidden" matTooltip="停止执行">
              <mat-icon>stop_circle</mat-icon>
            </button>
            <button mat-button color="warn" (click)="cancel()" class="hidden sm:inline-flex !rounded-full font-bold">
              <mat-icon>stop_circle</mat-icon>
              停止执行
            </button>
          }
      ...
      <div class="flex flex-1 flex-col sm:flex-row overflow-hidden">
        ...
        <aside [class.w-72]="!isHandset()" 
          ...
          <div class="px-6 py-3 hidden sm:flex justify-between items-center shrink-0">
            <span class="text-[10px] font-bold text-outline uppercase tracking-widest">流水线步骤</span>
            <span class="text-[9px] px-1.5 py-0.5 rounded bg-outline/10 text-outline font-mono">{{ stepsList().length + 1 }}</span>
          </div>
          
          <!-- Desktop List / Mobile Horizontal Scroll -->
          <div class="flex-1 overflow-x-auto sm:overflow-y-auto px-2 sm:px-3 py-2 sm:pb-4 flex sm:flex-col gap-1 no-scrollbar">
                <!-- System Setup -->
                <div (click)="onStepChange('')" 
                  ...
                  <mat-icon class="chevron hidden sm:block opacity-0 group-hover:opacity-100">chevron_right</mat-icon>
                </div>

                <!-- Workflow Steps -->
                @for (step of stepsList(); track step.id) {
                  <div (click)="onStepChange(step.id)" 
                       [class.active-step]="selectedStep() === step.id"
                       class="step-nav-item group shrink-0 sm:shrink">
                    <div class="status-indicator" [style.color]="getStepStatusColor(step.id)">
                      <mat-icon class="!w-4 !h-4 !text-[16px]">{{ getStepStatusIcon(step.id) }}</mat-icon>
                    </div>
                    <span class="text-[11px] sm:text-xs font-medium truncate flex-1">{{ step.name || step.id }}</span>
                    <mat-icon class="chevron hidden sm:block opacity-0 group-hover:opacity-100">chevron_right</mat-icon>
                  </div>
                }
          </div>
        ...
        <main class="flex-1 flex flex-col bg-[#1e1e1e] relative min-w-0">
          ...
          <div class="flex items-center justify-between px-4 py-2 bg-[#252526] border-b border-white/5 shrink-0">
            <div class="flex items-center gap-2">
                <mat-icon class="!w-3 !h-3 !text-[12px] text-primary">terminal</mat-icon>
                <span class="text-[9px] font-bold font-mono text-white/40 uppercase tracking-widest truncate">
          ...
          @if (instance().error && selectedStep() === '') {
            <div class="absolute bottom-4 left-4 right-4 p-3 bg-error/90 backdrop-blur-md rounded-xl text-on-error shadow-2xl flex items-start gap-3 animate-in slide-in-from-bottom-2">
