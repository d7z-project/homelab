    ...
    <mat-dialog-content>
      <form [formGroup]="form" class="flex flex-col gap-4 pt-2">
        ...
        <mat-form-field appearance="outline">
          ...
          <mat-hint>支持文本 (Site/Domain) 或 GeoSite (.mmdb, .dat) 文件</mat-hint>
        </mat-form-field>
        ...
        <div class="flex gap-4 mt-2">
          <mat-form-field appearance="outline" class="flex-1">
            ...
            <mat-select formControlName="format" required>
              <mat-option value="text">文本 (Text/Domain)</mat-option>
              <mat-option value="csv">CSV (Comma Separated)</mat-option>
              <mat-option value="geosite">GeoSite (V2Ray Dat / MMDB)</mat-option>
              <mat-option value="v2ray-dat">GeoSite (V2Ray Dat)</mat-option>
            </mat-select>
          </mat-form-field>
        ...
        >
          ...
          @if (form.get('format')?.value === 'csv') {
            <div class="grid grid-cols-3 gap-4">
              ...
              <mat-form-field appearance="outline">
                <mat-label>Site 列索引</mat-label>
                <input matInput type="number" formControlName="siteColumn" min="0" />
          ...
          @if (
            ...
            <div class="space-y-4">
              ...
              @if (!form.get('importAll')?.value) {
                <mat-form-field appearance="outline" class="w-full">
                  <mat-label>指定分类代码 (Category)</mat-label>
                  <input matInput formControlName="code" placeholder="如: cn, google" />
        ...
        <div class="py-2 flex gap-6">
          ...
          <mat-slide-toggle formControlName="allowPrivate">允许私有 Site</mat-slide-toggle>
        </div>
      </form>
    </mat-dialog-content>
...
export class CreateSyncPolicyDialogComponent implements OnInit {
  ...
  submit() {
    ...
    const obs = this.data.policy?.id
      ? this.siteService.networkSiteSyncIdPut(this.data.policy.id, policy)
      : this.siteService.networkSiteSyncPost(policy);
