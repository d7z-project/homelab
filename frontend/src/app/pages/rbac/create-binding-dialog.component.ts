  ...
  template: `
                ...
                <mat-form-field appearance="outline" class="w-full">
                  ...
                  <mat-select [(ngModel)]="binding.serviceAccountName" [disabled]="isEdit">
                    ...
                    <div *ngIf="hasMoreSa()" class="px-2 py-1">
                      <button mat-button class="w-full !text-xs !text-primary" (click)="$event.stopPropagation(); loadMoreSa()">
                        加载更多...
                      </button>
                    </div>
                  </mat-select>
        ...
        <mat-form-field appearance="outline" class="w-full">
          ...
          <mat-select [(ngModel)]="binding.roleNames" multiple>
            ...
            <div *ngIf="hasMoreRoles()" class="px-2 py-1">
              <button mat-button class="w-full !text-xs !text-primary" (click)="$event.stopPropagation(); loadMoreRoles()">
                加载更多...
              </button>
            </div>
          </mat-select>
    ...
    <mat-dialog-actions align="end" class="!px-6 !pb-6">
      ...
      <button mat-flat-button color="primary" [mat-dialog-close]="binding" 
              [disabled]="!binding.name || !binding.serviceAccountName || !binding.roleNames || binding.roleNames.length === 0 || (!isEdit && isDuplicate())" class="!ml-2">
        {{ isEdit ? '保存修改' : '确认创建' }}
      </button>
    </mat-dialog-actions>
  `,
...
export class CreateBindingDialogComponent implements OnInit {
  ...
  constructor(@Inject(MAT_DIALOG_DATA) public data: { binding?: AuthRoleBinding, existingNames?: string[] }) {
    if (data.binding) {
      this.isEdit = true;
  ...
  async fetchSa(reset = false) {
    ...
    const res = await firstValueFrom(this.rbacService.rbacServiceaccountsGet(this.saPage + 1, this.pageSize));
  ...
  async fetchRoles(reset = false) {
    ...
    const res = await firstValueFrom(this.rbacService.rbacRolesGet(this.rolePage + 1, this.pageSize));
    const items = res.items || [];
