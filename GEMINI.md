...
- **任务执行逻辑 (Executor)**:
  - **步骤快照**: 任务启动时必须将工作流步骤深度克隆至 `TaskInstance`，执行引擎仅允许从快照读取指令。
  - **身份模拟 (Impersonation)**: 任务执行阶段必须通过 `commonauth.WithAuth` 注入工作流指定的 `ServiceAccountID`。
...
- **执行状态反馈**: 关键业务操作（如“测试预览”）必须在按钮内部集成 `mat-spinner` 加载动画，确保在异步执行期间提供明确的视觉反馈。
...
- **标签排序逻辑**: 命中的标签列表必须执行 **系统标签优先排序**，即所有以 `_` 下划线开头的元数据标签始终显示在首位。
